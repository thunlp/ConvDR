...
for target in targets:
    ...
    with open(train, "r") as f, open(queries_manual, "w") as g, open(cqr, "w") as h:
        responses = []
        last_dialog_id = None
        for line in f:
            obj = json.loads(line)
            ...
            for his in obj["history"]:
                ...
                raw_query = his["question"]
            input_sents.append(obj["question"])
            h.write(json.dumps({"qid": str(idx), "input": input_sents, "target": query, "manual_response": responses}) + "\n")
            g.write(f"{idx}\t{query}\n")
...
for qid, v in qrels_dict.items():
    ...
    for pid in v.keys():
        ...
        target_qrels_file.write(f"{query_idx}\t0\t{passage_idx}\t1\n")

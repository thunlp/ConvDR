...
def train(args, train_dataset, model, teacher_model, loss_fn, logger, writer: SummaryWriter, cross_validate_id=-1, loss_fn_2=None, tokenizer=None):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, collate_fn=train_dataset.get_collate_fn(args, "train"))
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss1 = None
            if not args.no_mse:
                loss1 = loss_fn(embs, teacher_embs) if not args.use_kl else loss_fn(nn.functional.log_softmax(embs), nn.functional.softmax(teacher_embs))
            loss = loss1
            loss2 = None
            if args.ranking_task:
                ...
                for group in pos_and_negs:
                    ...
                    for doc in this_pos_neg:
                        doc_ids = tokenizer.encode(doc, add_special_tokens=True, max_length=args.max_seq_length)
                        doc_ids, doc_mask = pad_input_ids_with_mask(doc_ids, args.max_seq_length)
                ...
                for i in range(0, len(total_token_list), doc_batch_size):
                    ...
                    batch_ids = []
                    batch_mask = []
                    for j in range(i, min(i+doc_batch_size, len(total_token_list))):
                        batch_ids.append(total_token_list[j][0])
                        batch_mask.append(total_token_list[j][1])
                    batch_ids = torch.tensor(batch_ids, dtype=torch.long).to(args.device)
                    batch_mask = torch.tensor(batch_mask, dtype=torch.long).to(args.device)
                    with torch.no_grad():
                        pos_and_negs_embeddings_tmp = teacher_model(batch_ids, batch_mask).detach()
                        pos_and_negs_embeddings.append(pos_and_negs_embeddings_tmp)
                pos_and_negs_embeddings = torch.cat(pos_and_negs_embeddings, dim=0)  # (B * K, E)
                pos_and_negs_embeddings = pos_and_negs_embeddings.view(bs, args.num_negatives+1, -1)  # (B, K, E)
                embs_for_ranking = embs.unsqueeze(-1)  # (B, E, 1)
                embs_for_ranking = embs_for_ranking.expand(bs, 768, args.num_negatives+1)  # (B, E, K)
                embs_for_ranking = embs_for_ranking.transpose(1, 2)  # (B, K, E)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.log_steps == 0:
                    writer.add_scalar(str(cross_validate_id) + "/loss", tr_loss / args.log_steps, global_step)
                    if not args.no_mse:
                        writer.add_scalar(str(cross_validate_id) + "/mse_loss", tr_loss1 / args.log_steps, global_step)
                    if args.ranking_task:
                        writer.add_scalar(str(cross_validate_id) + "/ranking_loss", tr_loss2 / args.log_steps, global_step)
                    tr_loss = 0.0
                    tr_loss1 = 0.0
                    tr_loss2 = 0.0
...
def main():
    ...
    parser.add_argument("--ranking_task", action='store_true')
    ...
    loss_fn = nn.MSELoss() if not args.use_kl else nn.KLDivLoss()
    ...
    if not args.cross_validate:
        ...
        train_dataset = ConvSearchDataset([args.train_file], tokenizer, args, mode="train")
        global_step, tr_loss = train(args, train_dataset, model, teacher_model, loss_fn, logger, tb_writer, cross_validate_id=0, loss_fn_2=loss_fn_2)
        ...
        for i in range(NUM_FOLD):
            ...
            train_dataset = ConvSearchDataset(train_files, tokenizer, args, mode="train")
            global_step, tr_loss = train(args, train_dataset, model, teacher_model, loss_fn, logger, tb_writer, cross_validate_id=i, loss_fn_2=loss_fn_2, tokenizer=tokenizer)
            logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
